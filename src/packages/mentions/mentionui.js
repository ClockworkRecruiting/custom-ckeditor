...
import { Collection, keyCodes, env, Rect, CKEditorError, logWarning } from '@ckeditor/ckeditor5-utils';
...
export default class MentionUI extends Plugin {
	...
	_createMentionView() {
		...
		mentionsView.on( 'execute', ( evt, data ) => {
			...
			if (item.nestedFeedId) { // nested field id is present check for if it is clickable or not
				if (item.isClickable) this._requestFeedDebounced( marker, this.feedText, item );
	 ...
	 * @param {String} feedText
	 */
	_requestFeed( marker, feedText, item ) {
		// Store the last requested feed - it is used to discard any out-of order requests.
		this._lastRequested = feedText;

		const updateUiCallback = (response) => this.fire( 'requestFeed:response', { feed: response, marker, feedText } );
	...
	_handleFeedResponse( data ) {
		...
		const firstFocusIndex = feed.findIndex(a => !a.nestedFeedId);
	 ...
	 * @private
	 */
	_showOrUpdateUI( markerMarker, firstFocusIndex ) {
