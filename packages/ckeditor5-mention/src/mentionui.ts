...
export default class MentionUI extends Plugin {
	 ...
	 */
	private _requestFeedDebounced: ( marker: string, feedText: any, item?: any ) => void;
  ...
  isShowMentionSelector: boolean;
	...
	private _createMentionView(): MentionsView {
      ...
      if (item.nestedFeedId) { // nested field id is present check for if it is clickable or not
        ...
        if (item.isClickable) this._requestFeedDebounced( marker, feedText, item );
	 ...
	 */
	private _requestFeed( marker: string, feedText: string, item: any ): void {
    ...
    // :: CUSTOM_CODE_START
    const updateUiCallback = (response: Array<MentionFeedItem>) => this.fire<RequestFeedResponseEvent>( 'requestFeed:response', { feed: response, marker, feedText } );
    // :: CUSTOM_CODE_END

		const { feedCallback } = this._mentionsConfigurations.get( marker )!;
    const feedResponse = feedCallback(feedText, item, updateUiCallback);
    ...
    // :: CUSTOM_CODE_START
		const firstFocusIndex = feed.findIndex(a => typeof a === 'object' ? !a.nestedFeedId : '');
    // :: CUSTOM_CODE_END
      ...
      // :: CUSTOM_CODE_START
			this._showOrUpdateUI( mentionMarker!, firstFocusIndex );
       // :: CUSTOM_CODE_END
		} else {
	 ...
	 */
  // :: CUSTOM_CODE_START
	private _showOrUpdateUI( markerMarker: Marker, firstFocusIndex: number ): void {
  // :: CUSTOM_CODE_START
    ...
    // :: CUSTOM_CODE_START
		this._mentionsView.selectFirst(firstFocusIndex);
    // :: CUSTOM_CODE_END
	}
